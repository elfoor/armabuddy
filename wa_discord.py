...
class WA_Discord(discord.Client):
    ...
    async def create_gamelist(self, games: list):
        ...
        for game in games:
            ...
            append += ''.join((
                f'[{game["title"]}](',
                f'{self.http_redir_url}/',
                f'?Host={game["host"]}',
                f'&Port={game["port"]}' if game['port'] is not None else '',
                '&Scheme=Pf,Be',
                f'&ID={game["gameid"]}',
                ')\n',
    ...
    async def get_sorted_user_entries(self, all_users):
        ...
        for username, realname_parameters in all_users.items():
            # Treat ChanServ and HostingBuddy as snoopers to get them sorted at the bottom with the other bots
            if username == 'ChanServ':
                snoop_users.append((self.embed_snooper_icon, username))  # NOTE: not shown as a snooper in WA as of 3.8
                continue
            if username == 'HostingBuddy' and realname_parameters.endswith('HostingBuddy'):
            ...
            if realname_parameters[1] in ('13', '4294967309'):  # 13 + (1<<32)
                snoop_users.append((self.embed_snooper_icon, username))
                continue
